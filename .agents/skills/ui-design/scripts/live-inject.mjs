 ...
 *
 * On first live run, the agent generates `.uizze/live/config.json`
...
import { resolveLiveConfigPath } from "./lib/uizze-paths.mjs";
import {
...
const IGNORE_MARKER_OPEN = "# uizze-live-ignore-start";
const IGNORE_MARKER_CLOSE = "# uizze-live-ignore-end";

export const LIVE_IGNORE_PATTERNS = Object.freeze([
  ".uizze/hook.cache.json",
  ".uizze/hook.pending.json",
  ".uizze/config.local.json",
  ".uizze/live/server.json",
  ".uizze/live/roots.json",
  ".uizze/live/app-root.json",
  ".uizze/live/inject-journal.json",
  ".uizze/live/sessions/",
  ".uizze/live/previews/",
  ".uizze/live/annotations/",
  ".uizze/live/artifacts/",
  ".uizze/live/accept-receipts/",
  ".uizze/live/locks/",
  ".uizze/live/cache/",
  ".uizze/live/manual-edit-apply-transaction.json",
  ".uizze/live/manual-edit-events.jsonl",
  ".uizze/live/manual-edit-evidence/",
  ".uizze/live/pending-manual-edits.json",
  ".uizze/live/deferred-svelte-component-accepts.json",
  ".uizze-live.json",
  ".uizze-live/",
  "app/.uizze-live/",
  "src/.uizze-live/",
  "node_modules/.uizze-live/",
  "src/lib/uizze/UizzeLiveRoot.svelte",
  "src/lib/uizze/__runtime.js",
  "src/lib/uizze/[0-9a-f]*/",
  "plugins/uizze-live.client.ts",
  "app/plugins/uizze-live.client.ts",
  "src/plugins/uizze-live.client.ts",
]);
...
Reads configuration from .uizze/live/config.json.

Modes:
  --port PORT   Insert script tag pointing at http://localhost:PORT/live.js
  --remove      Remove the script tag (if present)
  --check       Print whether .uizze/live/config.json exists and its content
...
Output (JSON):
  ...
  if (!token) {
    try {
      const info = JSON.parse(
        fs.readFileSync(
          path.join(cwd, ".uizze", "live", "server.json"),
          "utf-8",
        ),
      );
  ...
  try {
    const rootsManifest = JSON.parse(
      fs.readFileSync(path.join(cwd, ".uizze", "live", "roots.json"), "utf-8"),
    );
    if (
