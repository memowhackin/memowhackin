#!/usr/bin/env node
/**
 * Uizze design hook — Cursor preToolUse write gate.
 *
...
async function detectProposedHtml(detector, content, filePath, scanOptions) {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "uizze-pre-"));
...
const BLOCK_PREFIX = "Uizze design hook blocked this write before it landed. ";

function cursorBlockMessage(
  findings,
  filePath,
  config,
  cwd,
  ...
  const rendered = renderTemplate(
    ...
    { ...config, limits: { ...limits, maxChars: budget } },
    {
      cwd,
      footer: footerMode,
      reserveChars: (reserveChars || 0) + BLOCK_PREFIX.length,
    },
  );
  return rendered.replace(
    "[uizze@1] Design hook findings requiring review",
    `[uizze@1] ${BLOCK_PREFIX}Design hook findings requiring review`,
  );
}
...
async function main() {
  if (truthy(process.env.UIZZE_HOOK_DISABLED)) {
  ...
  if (denial.count > EDIT_COUNT_THRESHOLD) {
    const warning = `${message}\n\nThis is the ${denial.count}th repeated denial for the same file and finding signature, so Uizze is allowing this write to avoid a loop. Reconsider the issue immediately after the tool runs.`;
    return allow(
      {
        ...audit,
        findings: (findings || []).length,
...
main().catch((err) => {
  if (process.env.UIZZE_HOOK_DEBUG) {
    process.stderr.write(`[uizze-hook-before-edit] ${err}\n`);
  }
