 ...
 *   1. Check .uizze/live/config.json (returns config_missing if first-ever run)
 ...
 *   - Open the project's live dev/preview URL in the browser (optional, if browser automation exists)—not `serverPort`; that port is the Uizze helper for /live.js and /poll
...
import { readLiveServerInfo } from "./lib/uizze-paths.mjs";
...
Prepare everything for live variant mode in a single command:
  - Checks .uizze/live/config.json (required, created once per project)
...
The agent should then:
      ...
      JSON.stringify(
        {
          ...
          ...targetSelection,
          hint: "Ask the user which app Uizze should use, then rerun live from that child app cwd. Use --target <path> only as a fallback or explicit path diagnostic.",
        },
        null,
        2,
      ),
  ...
  try {
    // Briefs live under .uizze/surfaces, which in a nested-app repo sits
