...
export async function wrapCli() {
  ...
  if (args.includes("--help") || args.includes("-h")) {
    console.log(`Usage: uizze wrap [options]
...
The agent should insert variant HTML at insertLine.`);
  ...
  const wrapperLines = isJsx
    ? [
        ...
        indent + '  <div data-uizze-variant="original">',
        reindentOriginal("    "),
        indent + "  </div>",
        indent +
          "  " +
...
function buildCssAuthoring(styleMode, count) {
  ...
  if (styleMode.mode === "astro-global-prefixed") {
    return {
      ...
      rulePattern: '[data-uizze-variant="N"] > .variant-class { ... }',
      selectorExamples: variantNumbers.map(
        (n) => `[data-uizze-variant="${n}"] > .variant-class`,
      ),
      requirements: [
        ...
        'Prefix every preview selector with the matching [data-uizze-variant="N"] selector.',
  ...
  return {
    ...
    strategy: "scope-rule",
    rulePattern:
      '@scope ([data-uizze-variant="N"]) { :scope > .variant-class { ... } }',
    selectorExamples: variantNumbers.map(
      (n) =>
        `@scope ([data-uizze-variant="${n}"]) { :scope > .variant-class { ... } }`,
    ),
    requirements: [
      'Use @scope blocks keyed to each [data-uizze-variant="N"] wrapper.',
    ...
    forbidden: [
      'Do not use global [data-uizze-variant="N"] selector prefixes for this styleMode.',
 ...
 *
 * Only `node_modules`, `.git`, and `.uizze` are skipped outright.
...
function findElement(lines, query, tag = null) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (lines[i].includes("data-uizze-variant")) continue;
