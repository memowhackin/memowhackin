 ...
 * `.uizze/live/config.json`. This module owns that block: building it,
...
export const MARKER_OPEN_TEXT = "uizze-live-start";
export const MARKER_CLOSE_TEXT = "uizze-live-end";

/** Markers that identify a file as still carrying our tag-strategy patch. */
export const TAG_PATCH_MARKERS = Object.freeze([
  MARKER_OPEN_TEXT,
  "data-uizze-csp-original",
]);
...
export function removeTag(content, _syntax) {
  const patterns = [
    /([ \t]*)<!--\s*uizze-live-start\s*-->[\s\S]*?<!--\s*uizze-live-end\s*-->([ \t]*(?:\r\n|\n|\r|$)?)/,
    /([ \t]*)\{\/\*\s*uizze-live-start\s*\*\/\}[\s\S]*?\{\/\*\s*uizze-live-end\s*\*\/\}([ \t]*(?:\r\n|\n|\r|$)?)/,
  ];
...
// and stash the original `content` value in a `data-uizze-csp-original`
...
const CSP_MARKER_ATTR = "data-uizze-csp-original";
