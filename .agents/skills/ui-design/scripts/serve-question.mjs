 ...
 * browser is available (UIZZE_QUESTION_DISABLED, or a detected
 * CI/headless/remote environment; UIZZE_QUESTION_FORCE=1 overrides
 ...
 *       "comp": ".uizze/mocks/decision/assigned.webp",  // optional; the card's
...
if (process.env.UIZZE_QUESTION_DISABLED) {
  console.log(
...
const wantsBrowser =
  ...
  !hasFlag("stop") &&
  !hasFlag("schema");
if (wantsBrowser && !process.env.UIZZE_QUESTION_FORCE) {
  const headless =
    process.env.CI ||
  ...
  if (headless) {
    console.log(
      "serve-question: no browser detected in this environment (CI/headless/remote); use the structured question tool instead. Set UIZZE_QUESTION_FORCE=1 to serve anyway.",
    );
    process.exit(2);
  }
}
  ...
  try {
    ...
    if (a.buildPath === "comp" || a.buildPath === "code") {
      ...
      const origin = a.buildPathFlipped
        ? "flipped on the page, so it binds this session only, and the page never writes it back; the sole exception is new-work’s one-time offer, on a project that had no recorded default at all, which asks after the round closes and writes the answer to .uizze/config.json"
...
const QUESTION_DIR = path.join(process.cwd(), ".uizze", "questions");
    ...
    JSON.stringify(
      {
        ...
        options: [
          {
            ...
            comp: ".uizze/mocks/decision/assigned.webp",
            hero: "https://uizze.style/worlds/cards/posters-covers-sleeves-fillmore-handbill-hero.webp",
            board:
              "https://uizze.style/worlds/cards/posters-covers-sleeves-fillmore-handbill.webp",
          },
          {
            id: "model-pick",
            ...
            comp: ".uizze/mocks/decision/model-pick.webp",
          },
          {
            ...
            comp: ".uizze/mocks/decision/challenger-teletext.webp",
            hero: "https://uizze.style/worlds/cards/broadcast-programming-teletext-service-hero.webp",
          },
          {
            ...
            hero: "https://uizze.style/worlds/cards/archives-microfiche-reader-hero.webp",
          },
        ],
        ...
        canonCard: {
          ...
          comp: ".uizze/mocks/decision/canon.webp",
        },
        steer: true,
      },
      null,
...
if (hasFlag("wait")) {
  ...
  try {
    ...
    keepsTableOpen =
      parsedAnswer.optionId === "reroll" || parsedAnswer.followup === true;
  } catch {
    /* treat as terminal */
  }
  try {
...
function page() {
  ...
  const faceComp = (option) =>
    demoted(option) || codeLed ? null : option.compSrc;
  ...
  const anatomy = (option) => {
    ...
    if (Array.isArray(option.raised) && option.raised.length) {
      ...
      const raisesHead = (count) =>
        `<div class="raises-head"><span class="fact-label">Improved by Uizze's worlds</span>${count > 1 ? `<span class="raises-count" data-raises-count>1/${count}</span>` : ""}</div>`;
      if (raiseLines.length > 1) {
        rows.push(`<div class="raises raises-cycle" role="button" tabindex="0" title="Click or press Enter for the next improvement" aria-label="How Uizze's worlds improved this direction; activate to see the next improvement">
  ...
  const wire = (option) => {
    ...
    const pct = (n, total) =>
      `${Math.max(0, Math.min(100, (n / total) * 100)).toFixed(2)}%`;
...
<title>${esc(payload.title || "uizze · decision")}</title>
...
<style>
  /* Neo kinpaku tokens, mirrored from uizze.style kinpaku-tokens.css */
  :root {
    color-scheme: dark;
...
<header>
  <div class="brand">
    ...
    <span class="wordmark">Uizze</span>
  </div>
</header>
<main>
  <div class="stage">
...
const server = http.createServer((req, res) => {
  ...
  if (req.method === "POST" && req.url === "/answer") {
    ...
    req.on("end", () => {
      ...
      const followupOpen =
        Boolean(detachedKey) && payload.followup === true && !isReroll;
      const answer = JSON.stringify({
        optionId: parsed.optionId ?? null,
        steer: parsed.steer ?? "",
        ...(isReroll &&
        (parsed.register === "safer" || parsed.register === "bolder")
