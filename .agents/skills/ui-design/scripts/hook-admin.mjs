#!/usr/bin/env node
/**
 * The Uizze hooks command manages the design hook runtime
 * via the `hook` key and shared detector ignores via the `detector` key in
 * .uizze/config.json / .uizze/config.local.json.
 *
 * Usage:
...
import { UIZZE_COMMAND } from "./lib/provider.mjs";
...
const UIZZE_HOOK_COMMAND_MARKERS = [
  "skills/uizze/scripts/hook-probe.mjs",
  "skills/uizze/scripts/hook.mjs",
  "skills/uizze/scripts/hook-before-edit.mjs",
  "skills/uizze/scripts/hook-after-edit.mjs",
  "skills/uizze/scripts/hook-stop.mjs",
];
...
const HOOK_MANIFEST_TARGETS = [
  {
    provider: ".claude",
    skillRel: ".claude/skills/uizze",
    destRel: ".claude/settings.local.json",
    sharedDestRel: ".claude/settings.json",
    manifest: () => ({
      description:
        "Uizze design detector: immediate-tier checks after Edit/Write/MultiEdit on UI files, full-rule deep pass on Stop.",
      hooks: {
        PostToolUse: [
          {
            ...
            hooks: [
              {
                type: "command",
                command:
                  'node "${CLAUDE_PROJECT_DIR}/.claude/skills/uizze/scripts/hook.mjs"',
  ...
  },
  {
    ...
    manifest: () => ({
      hooks: {
        PostToolUse: [
          {
            ...
            hooks: [
              {
                type: "command",
                command: 'node ".agents/skills/uizze/scripts/hook.mjs"',
                timeout: TIMEOUT_SECONDS,
                statusMessage: STATUS_MESSAGE,
              },
            ],
        ...
        Stop: [
          stopManifestEntry('node ".agents/skills/uizze/scripts/hook.mjs"'),
        ],
      },
    }),
  },
  {
    ...
    manifest: () => ({
      ...
      hooks: {
        preToolUse: [
          {
            command: 'node ".cursor/skills/uizze/scripts/hook-before-edit.mjs"',
            timeout: TIMEOUT_SECONDS,
          },
        ],
      },
  ...
  },
  {
    ...
    provider: ".github",
    skillRel: ".github/skills/uizze",
    destRel: ".github/hooks/uizze.json",
    manifest: () => ({
      version: 1,
      hooks: {
        postToolUse: [
          {
            ...
            bash: 'node "$(git rev-parse --show-toplevel)/.github/skills/uizze/scripts/hook.mjs"',
...
function writeHookConfig(cwd, hookConfig, opts = {}) {
  ...
  // (consent, quiet, auditLog) survive an Uizze hooks edit.
...
function statusReport(cwd) {
  ...
  const envKill = process.env.UIZZE_HOOK_DISABLED;
  const envState = envKill ? `UIZZE_HOOK_DISABLED=${envKill}` : "unset";
  const cfgPath =
    path.relative(cwd, getConfigPath(cwd)) || ".uizze/config.json";
  const localPath =
    path.relative(cwd, getLocalConfigPath(cwd)) || ".uizze/config.local.json";
  const cachePath =
    path.relative(cwd, getCachePath(cwd)) || ".uizze/hook.cache.json";
  ...
  // `rule=value [files]` shape `uizze ignores list` already prints.
...
function repairHookManifests(cwd) {
  ...
  for (const target of HOOK_MANIFEST_TARGETS) {
    ...
    if (sharedDest && fileHasUizzeHookMarker(sharedDest)) {
      pruneUizzeHookFromManifest(dest);
...
function mergeHookManifests(existing, fresh) {
  ...
  for (const event of hookEvents) {
    const preserved = stripUizzeHookEntries(existingHooks[event]);
...
function fileHasUizzeHookMarker(filePath) {
  ...
  return valueHasUizzeHookMarker(parsed.hooks);
}

function valueHasUizzeHookMarker(value) {
  if (typeof value === "string") {
    return UIZZE_HOOK_COMMAND_MARKERS.some((marker) => value.includes(marker));
  }
  if (Array.isArray(value)) return value.some(valueHasUizzeHookMarker);
  if (value && typeof value === "object")
    return Object.values(value).some(valueHasUizzeHookMarker);
  return false;
}

function stripUizzeHookEntry(entry) {
  ...
  if (
    valueHasUizzeHookMarker(entry.command) ||
    valueHasUizzeHookMarker(entry.args) ||
    valueHasUizzeHookMarker(entry.bash) ||
    valueHasUizzeHookMarker(entry.powershell)
  ) {
    return null;
  }
  if (!Array.isArray(entry.hooks)) return entry;

  const strippedHooks = entry.hooks.map(stripUizzeHookEntry).filter(Boolean);

  if (strippedHooks.length === 0 && entry.hooks.some(valueHasUizzeHookMarker)) {
    return null;
  }
  return { ...entry, hooks: strippedHooks };
}

function stripUizzeHookEntries(entries) {
  if (!Array.isArray(entries)) return [];
  return entries.map(stripUizzeHookEntry).filter(Boolean);
}

function pruneUizzeHookFromManifest(manifestPath) {
  if (!fileHasUizzeHookMarker(manifestPath)) return false;
  let parsed;
  try {
  ...
  for (const [event, entries] of Object.entries(existingHooks)) {
    const kept = stripUizzeHookEntries(entries);
...
function addIgnoreRule(cwd, args) {
  ...
  if (!rule)
    throw new Error(
      `Pass a rule id, e.g. ${UIZZE_COMMAND} hooks ignore-rule side-tab`,
    );
  if (rule === "overused-font" && !parsed.allValues) {
    throw new Error(
      `overused-font is value-specific by default. Use ${UIZZE_COMMAND} hooks ignore-value overused-font <font> for a confirmed font, or ${UIZZE_COMMAND} hooks ignore-rule overused-font --all-values only when the user asked to ignore overused fonts generally.`,
    );
  }
...
function addIgnoreFile(cwd, args) {
  ...
  if (!glob)
    throw new Error(
      `Pass a glob, e.g. ${UIZZE_COMMAND} hooks ignore-file "src/legacy/**"`,
    );
...
function parseIgnoreValueArgs(args) {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (arg === "--shared") {
      ...
      // reported success. Matches `uizze ignores add-value`.
...
function addIgnoreValue(cwd, args) {
  ...
  if (!parsed.rule || !parsed.value) {
    throw new Error(
      `Pass a rule id and value, e.g. ${UIZZE_COMMAND} hooks ignore-value overused-font Inter`,
    );
  }
  ...
  // `files` scope, so require one — matching `uizze ignores add-value`.
  if (parsed.value === "*" && parsed.files.length === 0) {
    ...
    const projectWide =
      parsed.rule === "overused-font"
        ? `${UIZZE_COMMAND} hooks ignore-rule ${parsed.rule} --all-values`
        : `${UIZZE_COMMAND} hooks ignore-rule ${parsed.rule}`;
    throw new Error(
      `Wildcard value ignores must be scoped with --file <glob>, e.g. ${UIZZE_COMMAND} hooks ignore-value design-system-font-size "*" --file "src/widget.js". To suppress the rule project-wide use ${projectWide}.`,
    );
  }
