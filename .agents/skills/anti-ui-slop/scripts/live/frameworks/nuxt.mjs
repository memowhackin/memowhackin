...
export const NUXT_PLUGIN_MARKER = "uizze-live-nuxt-plugin";
export const NUXT_PLUGIN_NAME = "uizze-live.client.ts";
...
const liveSelector = 'script[data-uizze-live-nuxt]';
...
export function applyNuxtLiveAdapter({
  ...
  if (existing !== null && !existing.includes(NUXT_PLUGIN_MARKER)) {
    return {
      ...
      hint: `${project.pluginFile} already exists and is not managed by Uizze Live`,
    };
  }
...
export function removeNuxtLiveAdapter({
  ...
  if (!content.includes(NUXT_PLUGIN_MARKER)) {
    return {
      ...
      removed: false,
      error: "nuxt_plugin_conflict",
      hint: `${project.pluginFile} is not managed by Uizze Live`,
    };
  }
