...
export const SVELTE_LIVE_ROOT_COMPONENT = "src/lib/uizze/UizzeLiveRoot.svelte";
export const SVELTE_LAYOUT_MARKER_OPEN = "<!-- uizze-live-svelte-start -->";
export const SVELTE_LAYOUT_MARKER_CLOSE = "<!-- uizze-live-svelte-end -->";
export const SVELTE_ROOT_IMPORT =
  "import UizzeLiveRoot from '$lib/uizze/UizzeLiveRoot.svelte';";
...
const SVELTE_ROOT_IMPORT_LINE_RE =
  /^[ \t]*import UizzeLiveRoot from '\$lib\/uizze\/UizzeLiveRoot\.svelte(?:\?[^']*)?';[ \t]*\r?\n?/gm;
...
export function svelteRootImportLine(rev) {
  ...
  return (
    "import UizzeLiveRoot from '$lib/uizze/UizzeLiveRoot.svelte?uizze-live=" +
    rev +
    "';"
  );
}
...
export function patchSvelteLayout(content, { rev = null } = {}) {
  ...
  if (!out.includes(importLine)) {
    ...
    // indentation; only a layout with no uizze import gets an insert.
    let replaced = false;
...
export function buildSvelteLiveRootComponent(port, token) {
  ...
  const HOST_ID = 'uizze-live-root';

  onMount(() => {
    let host = document.querySelector('uizze-live-root#' + HOST_ID) || document.getElementById(HOST_ID);
    if (!host) {
      host = document.createElement('uizze-live-root');
      host.id = HOST_ID;
      document.body.appendChild(host);
    }

    host.dataset.uizzeLiveAdapter = 'sveltekit';
    ...
    if (!root.querySelector('style[data-uizze-live-reset]')) {
      const reset = document.createElement('style');
      reset.dataset.uizzeLiveReset = 'true';
      reset.textContent = ':host, :host *, * { box-sizing: border-box; }';
      root.appendChild(reset);
    }

    window.__UIZZE_LIVE_ADAPTER__ = 'sveltekit';
    window.__UIZZE_LIVE_UI_ROOT__ = root;
    window.__UIZZE_LIVE_CHROME_MOUNT__ = {
      adapter: 'sveltekit',
      version: 1,
      host,
      root,
    ...
    script.src = LIVE_URL;
    script.async = true;
    script.dataset.uizzeLiveScript = 'true';
    script.onerror = () => console.error(
      '[uizze] live.js failed to load from ' + LIVE_URL
    ...
    return () => {
      script.remove();
      if (window.__UIZZE_LIVE_UI_ROOT__ === root) delete window.__UIZZE_LIVE_UI_ROOT__;
      if (window.__UIZZE_LIVE_CHROME_MOUNT__?.root === root) delete window.__UIZZE_LIVE_CHROME_MOUNT__;
      if (window.__UIZZE_LIVE_ADAPTER__ === 'sveltekit') delete window.__UIZZE_LIVE_ADAPTER__;
    };
  });
</script>
`;
