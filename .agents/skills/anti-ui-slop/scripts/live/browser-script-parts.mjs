...
export function assembleLiveBrowserScript({
  ...
  const prelude =
    `window.__UIZZE_TOKEN__ = '${token}';\n` +
    `window.__UIZZE_PORT__ = ${port};\n` +
    ...
    `window.__UIZZE_APP_ROOT__ = ${JSON.stringify(appRoot)};\n` +
    `window.__UIZZE_COMMAND_PREFIX__ = ${JSON.stringify(commandPrefix)};\n` +
    ...
    `window.__UIZZE_VOCAB__ = ${JSON.stringify(vocabulary)};\n` +
    ...
    // repo's tests, the uizze-site Live UI lab) import the module directly,
    // which is what keeps the two from drifting.
    `window.__UIZZE_LIVE_UI_SURFACES__ = ${JSON.stringify(uiSurfaces)};\n` +
    `window.__UIZZE_LIVE_MOUNT_CONTRACT__ = ${JSON.stringify(mountContract)};\n`;

  const body = parts
    .map((part) => {
      const file = part.file || path.basename(part.path || "");
      return `// --- uizze live script part: ${part.name} (${file}) ---\n${part.source}`;
    })
    .join("\n");
