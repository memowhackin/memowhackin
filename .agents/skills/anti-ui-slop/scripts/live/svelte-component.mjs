 ...
 *
 * Variants are real .svelte components under node_modules/.uizze-live/<session-id>/.
...
// node_modules, so an .uizze/ tree under the app root 403s (verified
...
export const SVELTE_COMPONENT_ROOT = "node_modules/.uizze-live";
// A short-lived interim location; swept so no project keeps a stray tree.
export const LEGACY_SVELTE_COMPONENT_ROOT = ".uizze/live/previews";
...
export const DEFERRED_ACCEPTS_FILE =
  ".uizze/live/deferred-svelte-component-accepts.json";

const MUSTACHE_RE = /\{([^{}]+)\}/g;

export function shouldUseSvelteComponentInjection(filePath) {
  if (/^(0|false|no)$/i.test(process.env.UIZZE_LIVE_SVELTE_COMPONENT || ""))
    return false;
...
export function ensureRuntimeHelper(cwd = process.cwd()) {
  ...
  if (!fs.existsSync(probe)) {
    fs.writeFileSync(
      probe,
      `export const uizzeLivePreviewProbe = true;\n`,
      "utf-8",
    );
  }
  return file;
...
function buildInsertVariantStub(variantNum) {
  return `${buildPropsScript([])}<div class="uizze-insert-preview">Insert variant ${variantNum}</div>\n\n<style>\n  .uizze-insert-preview { display: block; }\n</style>\n`;
}
...
function sanitizeAcceptedSvelteCss(
  ...
  if (!/data-uizze-variant|uizze-variant-ready/.test(css)) return cssLines;
...
function appendSanitizedCssRule(
  ...
  if (!prelude || !body || /--uizze-variant-ready\s*:/.test(body)) return;

  if (/^@scope\b/i.test(prelude)) {
    if (
      /data-uizze-variant/.test(prelude) &&
    ...
    for (const innerRule of inner) {
      ...
      if (!rewrittenPrelude || /--uizze-variant-ready\s*:/.test(innerRule.body))
        continue;
...
function variantSelectorRegex(variantNum) {
  return new RegExp(
    `\\[data-uizze-variant=(["'])${escapeRegExp(String(variantNum))}\\1\\]`,
    "g",
  );
}
...
export function inlineSvelteComponentAccept(
  ...
  if (/data-uizze-variant|uizze-variant-ready/.test(variantCss)) {
...
function inlineSvelteComponentInsertAccept({
  ...
  if (/\bdata-uizze-[\w-]*\s*=/.test(markup)) {
    return {
      handled: false,
      error:
        "Accepted Svelte insert variant contains preview-only data-uizze attributes",
      ...resultBase,
    };
  }
...
/**
 * Stop-path sweep. The whole `node_modules/.uizze-live` tree is
 * uizze-owned and gitignored, so once no session should survive there is
...
export function buildSvelteComponentCssAuthoring(count) {
  ...
  return {
    ...
    requirements: [
      ...
      'Declare params in componentDir/params.json keyed by variant number (e.g. {"1": [...], "2": [...]}), NOT as a data-uizze-params attribute.',
      "Do not use @scope or data-uizze-variant selectors in component files.",
    ...
    forbidden: [
      ...
      "Do not add data-uizze-* attributes inside component files. Svelte parses { in attribute values as an expression, so data-uizze-params with JSON breaks the build; use componentDir/params.json instead.",
    ],
