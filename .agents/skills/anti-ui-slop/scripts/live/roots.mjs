 ...
 *   sessionRoot <appRoot>/.uizze/live — durable live state.
 *
 ...
 *
 * The resolved manifest is persisted at <appRoot>/.uizze/live/roots.json
 * plus a pointer at <repoRoot>/.uizze/live/app-root.json when the two
...
function isAppRoot(dir) {
  ...
  return (
    hasDevConfig(dir) || exists(path.join(dir, ".uizze", "live", "config.json"))
  );
}
...
export function resolveRoots({ cwd = process.cwd(), targetPath = null } = {}) {
  ...
  return {
    manifest: {
      ...
      contextRoot,
      sessionRoot: path.join(appRoot, ".uizze", "live"),
      productPath,
      designPath,
      resolvedFrom,
    },
...
function rootsFilePath(appRoot) {
  return path.join(appRoot, ".uizze", "live", ROOTS_FILE);
}

function pointerFilePath(repoRoot) {
  return path.join(repoRoot, ".uizze", "live", POINTER_FILE);
}
...
function hasLiveServer(appRoot) {
  ...
  try {
    const info = JSON.parse(
      fs.readFileSync(
        path.join(appRoot, ".uizze", "live", "server.json"),
        "utf-8",
      ),
    );
...
function hasActiveDurableSession(appRoot) {
  const dir = path.join(appRoot, ".uizze", "live", "sessions");
  let entries;
  try {
 ...
 *
 *   1. Walk up from cwd looking for .uizze/live/roots.json.
 *   2. At the git root, follow .uizze/live/app-root.json to the app.
      ...
      if (candidates.length > 0) {
        ...
        if (tier.length > 1) {
          ...
          process.stderr.write(
            `[uizze live] Multiple apps in this repo have live state; using ${chosen}. ` +
...
export function enterLiveRoot(cwd = process.cwd()) {
  ...
  try {
    ...
    console.error(`[uizze live] ${err.message}`);
    process.exit(1);
  }
  ...
  if (path.resolve(cwd) !== path.resolve(appRoot)) {
    ...
    if (!isDir(appRoot)) {
      console.error(
        `[uizze live] resolved app root does not exist: ${appRoot} (stale roots manifest? re-run the live boot, or pass --target <path>)`,
      );
      process.exit(1);
    }
    try {
      ...
      console.error(
        `[uizze live] could not enter app root ${appRoot}: ${err.message}`,
      );
      process.exit(1);
    }
  }
