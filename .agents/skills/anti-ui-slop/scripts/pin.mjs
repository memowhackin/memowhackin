 ...
 *
 * `pin audit` creates a lightweight audit skill that redirects to Uizze's audit workflow.
...
const PIN_MARKER = "<!-- uizze-pinned-skill -->";
...
/**
 * Find harness skill directories that have an uizze skill installed.
 */
function findHarnessDirs(projectRoot) {
  const dirs = [];
  for (const harness of HARNESS_DIRS) {
    const skillsDir = join(projectRoot, harness, "skills");
    // Only pin in harness dirs that already have uizze installed
    const uizzeDir = join(skillsDir, "uizze");
    if (existsSync(uizzeDir) || existsSync(join(skillsDir, "i-uizze"))) {
      dirs.push(skillsDir);
    }
  }
  return dirs;
...
function generatePinnedSkill(command, metadata, commandPrefix, isCodex) {
  const desc =
    ...
    `Shortcut for ${commandPrefix}uizze ${command}.`;
...
This is a pinned shortcut for \`${commandPrefix}uizze ${command}\`.

Invoke ${commandPrefix}uizze ${command}, passing along any arguments provided here, and follow its instructions.
`;
}
...
function pin(command, projectRoot) {
  ...
  if (harnessDirs.length === 0) {
    console.log("No harness directories with uizze installed found.");
    return false;
  }
  ...
  for (const skillsDir of harnessDirs) {
    ...
    const content = generatePinnedSkill(
      command,
      metadata,
      commandPrefix,
      commandPrefix === "$",
...
function unpin(command, projectRoot) {
  ...
  if (removed > 0) {
    ...
    console.log(`Use Uizze's '${command}' workflow directly to access it.`);
  } else {
