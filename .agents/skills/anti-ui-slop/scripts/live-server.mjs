...
import {
  ...
  UIZZE_COMMAND_PREFIX,
  readLiveServerInfo,
  removeLiveServerInfo,
  resolveDesignSidecarPath,
  writeLiveServerInfo,
} from "./lib/uizze-paths.mjs";
...
// that outlives an `uizze document` run (or a context file created after
...
const DEBUG_MANUAL_EDIT_EVENTS = /^(1|true|yes)$/i.test(
  process.env.UIZZE_LIVE_DEBUG_EVENTS || "",
);
  ...
  return (req, res) => {
    ...
    if (p === "/live.js") {
      ...
      const body = assembleLiveBrowserScript({
        ...
        commandPrefix: UIZZE_COMMAND_PREFIX,
        appRoot: process.cwd(),
        parts,
      });
      res.writeHead(200, {
    ...
    //   /design-system.json    returns both parsed DESIGN.md and .uizze/design.json
    ...
    //                          - sidecar: .uizze/design.json contents when present.
      ...
      if (jsonStat) {
        try {
          ...
          response.sidecarError =
            "Failed to parse .uizze/design.json: " + err.message;
        }
      }
...
function sessionFileMetadataFromPollReply(file) {
  ...
  if (
    ...
    return base;
  if (
    !metadataFile.includes(".uizze/live/previews/") &&
    !metadataFile.includes("node_modules/.uizze-live/") &&
    !metadataFile.includes("src/lib/uizze/") &&
    !metadataFile.includes("/.uizze-live/")
  )
    return base;
...
function cleanupSvelteComponentSessionsBeforeExit() {
  try {
    ...
    console.warn(
      "[uizze] Svelte component session cleanup failed:",
      err.message,
    );
  }
}
...
function sweepOrphanSvelteComponentSessionsOnStartup() {
  try {
    ...
    if (result.removed.length > 0 || result.removedRoot) {
      console.log(
        "[uizze] swept orphaned Svelte component sessions:",
        JSON.stringify(result),
      );
    }
  } catch (err) {
    console.warn("[uizze] Svelte component session sweep failed:", err.message);
  }
}
...
function sweepStaleAcceptReceiptsOnStartup() {
  try {
    ...
    if (removed > 0)
      console.log(
        `[uizze] removed ${removed} accept receipt(s) older than 14 days`,
      );
  } catch (err) {
    console.warn("[uizze] accept receipt retention sweep failed:", err.message);
  }
}
...
function applyLegacyDeferredAcceptsOnStartup() {
  try {
    ...
    if (result.applied > 0 || result.failed > 0) {
      console.log(
        "[uizze] applied legacy deferred Svelte component accepts:",
        JSON.stringify(result),
      );
    }
  } catch (err) {
    console.warn(
      "[uizze] legacy deferred Svelte component accept apply failed:",
      err.message,
    );
  }
}
...
httpServer.listen(state.port, "127.0.0.1", () => {
  ...
  console.log(`\nUizze live server running on ${url}`);
