#!/usr/bin/env node
/**
 * Deep staleness pass over Uizze's own project artifacts.
 *
...
function usage() {
  return [
    ...
    "",
    "Report drift between this project's Uizze artifacts and what the",
    "installed version reads: PRODUCT.md, DESIGN.md and its sidecar,",
    ".uizze/config.json, surface briefs, and the design hook.",
    "",
  ...
  for (const name of ["config.json", "config.local.json"]) {
    try {
      const raw = JSON.parse(
        fs.readFileSync(path.join(repoRoot, ".uizze", name), "utf-8"),
      );
...
function renderText(report, fixes) {
  ...
  lines.push(`Uizze doctor: ${rel(report.projectRoot, process.cwd()) || "."}`);
  ...
  if (fixes) {
    ...
    lines.push(
      `Run \`node doctor.mjs --fix\` to apply the automatic migrations, ` +
        `or \`${UIZZE_COMMAND} doctor\` to work through all of them.`,
    );
  }
...
if (invokedAsScript()) {
  cli().catch((err) => {
    process.stderr.write(`uizze doctor failed: ${err?.message || err}\n`);
    process.exit(1);
  });
}
