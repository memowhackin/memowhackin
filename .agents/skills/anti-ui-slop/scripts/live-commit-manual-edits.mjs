 ...
 *
 * The browser Save path stages copy edits in .uizze/live. This script is
...
function repairAttemptLimit(env = process.env) {
  const value = Number(
    env.UIZZE_LIVE_MANUAL_EDIT_REPAIR_ATTEMPTS || DEFAULT_REPAIR_ATTEMPTS,
  );
...
async function main() {
  ...
  const result = await commitManualEdits({
    ...
    timeoutMs: Number(process.env.UIZZE_LIVE_COPY_AGENT_TIMEOUT_MS || 120000),
  });
