 ...
 *      `.uizze/config.json` declares `projectRoots`)
 *   4. $UIZZE_CONTEXT_DIR (absolute or cwd-relative) — power-user
...
import { UIZZE_COMMAND, UIZZE_PROVIDER_ID } from "./lib/provider.mjs";
...
// can offer `reinstall the current Uizze skill archive`. Everything here is best-effort and
// silent on failure: a network problem, sandbox, or missing cache must never
// block context output or print an error.

const UPDATE_HOST = (
  process.env.UIZZE_UPDATE_HOST || "https://uizze.style"
).replace(/\/$/, "");
const UPDATE_CACHE_PATH =
  process.env.UIZZE_UPDATE_CACHE ||
  path.join(os.homedir(), ".uizze", "update-check.json");
...
function resolveEnvContextDir(cwd) {
  const envDir = process.env.UIZZE_CONTEXT_DIR;
...
// outside both the project and repo roots (UIZZE_CONTEXT_DIR override).
...
// itself. Uizze `projectRoots` patterns govern every path they match:
...
// deeper folder would silently resolve there. Paths the Uizze group does
...
function isSelectableCandidate(repoRoot, rel, patternGroups) {
  ...
  const [uizzePatterns, packagePatterns] = patternGroups;
  if (isExcludedByWorkspacePattern(relSegments, uizzePatterns)) return false;
  for (const pattern of uizzePatterns) {
...
// `projectRoots` globs in .uizze config, then package-manager workspace
// declarations. A path matched by any Uizze pattern — positive or
// negated — is governed by the Uizze group alone; package-manager
// patterns only apply to paths the Uizze group does not match. Within a
// group, negations win over positives.
function readProjectPatternGroups(repoRoot) {
  return [
    readUizzeProjectRoots(repoRoot),
    [
...
function readUizzeProjectRoots(repoRoot) {
  const patterns = [];
  for (const name of ["config.json", "config.local.json"]) {
    const cfg = readJson(path.join(repoRoot, ".uizze", name));
...
function buildUpdateDirective(localVersion, latestVersion) {
  return (
    `UPDATE_AVAILABLE: A newer Uizze skill is available ` +
    `(installed v${localVersion}, latest v${latestVersion}). ` +
    `Mention it once, in this form: "A newer Uizze (v${latestVersion}) is available. ` +
    `Update now? It runs \`reinstall the current Uizze skill archive\`." ` +
    `Do not run \`reinstall the current Uizze skill archive\` in this turn, whatever the user answers: it rewrites the skill files ` +
 ...
 * the agent never nags. Opt out entirely with UIZZE_NO_UPDATE_CHECK=1.
 */
  ...
  for (const name of ["config.json", "config.local.json"]) {
    try {
      const raw = JSON.parse(
        fs.readFileSync(path.join(cwd, ".uizze", name), "utf-8"),
      );
      if (
        raw &&
        typeof raw === "object" &&
...
async function computeUpdateDirective(now = Date.now()) {
  try {
    if (process.env.UIZZE_NO_UPDATE_CHECK) return null;
    ...
    const parts = ctx.hasVisualImplementation
      ? [
          "NO_PRODUCT_MD: This project has no PRODUCT.md yet, but it does have an incumbent visual implementation. " +
            ...
            `offer \`${UIZZE_COMMAND} init\` as a follow-up.`,
      ...
      : [
          "NO_PRODUCT_MD: This project has no PRODUCT.md yet. " +
            ...
            `context and offer \`${UIZZE_COMMAND} init\` as a suggestion (do not block).`,
...
const HOOK_MANIFESTS_BY_PROVIDER = Object.freeze({
  ...
  github: [".github/hooks/uizze.json"],
  grok: [".grok/hooks/uizze.json"],
});
...
function valueHasHookMarker(value) {
  if (typeof value === "string") {
    return (
      value.includes("skills/uizze/scripts/hook.mjs") ||
      value.includes("skills/uizze/scripts/hook-before-edit.mjs")
    );
  }
...
function hookEnabledAt(root) {
  if (truthyEnv(process.env.UIZZE_HOOK_DISABLED)) return false;
  let enabled = true;
  for (const name of [".uizze/config.json", ".uizze/config.local.json"]) {
...
function automaticHookMode(ctx) {
  ...
  const manifests = HOOK_MANIFESTS_BY_PROVIDER[UIZZE_PROVIDER_ID] || [];
  const roots = [
    ...new Set(
  ...
  for (const root of roots) {
    for (const rel of manifests) {
      ...
      if (raw?.hooks && valueHasHookMarker(raw.hooks)) {
        return STOP_REVIEW_PROVIDERS.has(UIZZE_PROVIDER_ID)
          ? "stop"
          : "per-edit";
      }
    }
...
function readBuildPathAt(root) {
  ...
  let source = null;
  for (const name of ["config.json", "config.local.json"]) {
    const raw = readJson(path.join(root, ".uizze", name));
...
function appendBuildPathDirective(parts, ctx) {
  const roots = [
    ...new Set(
      [ctx?.projectRoot || process.cwd(), ctx?.repoRoot]
        .filter(Boolean)
        .map((root) => path.resolve(root)),
    ),
  ];
  for (const root of roots) {
    ...
    parts.push(
      `BUILD_PATH_DEFAULT: ${found.value} (from ${found.source}). Author direction and surface rounds with this as buildPath.value and toggle: true; a flip on the page binds that session only and is never written back, because a default is already recorded here. New-work's one-time offer to record a flipped value applies only where no default exists, which is why you are not seeing this line on those projects.`,
    );
    return;
  }
}
...
function appendAutonomyCounterDirective(parts) {
  parts.push(
    [
      ...
      "Uizze's interview and decision steps stay live: probe once with the structured question tool or the decision page.",
...
function appendDetectorFallback(parts, ctx) {
  ...
  parts.push(
    [
      "MANUAL_DETECTOR_REQUIRED: No automatic Uizze design hook is active this session.",
...
// Tier 1 staleness: schema drift in Uizze's own project files, measured
...
function projectRootsDiagnostic(ctx, options) {
  ...
  if (path.resolve(ctx.projectRoot || "") !== path.resolve(ctx.repoRoot))
    return {};
  const patterns = readUizzeProjectRoots(ctx.repoRoot);
...
function buildTargetSelectionDirective(selection) {
  return (
    ...
    "Ask the user which app Uizze should use, then rerun Uizze helper commands from that child app cwd using this same scripts directory. " +
