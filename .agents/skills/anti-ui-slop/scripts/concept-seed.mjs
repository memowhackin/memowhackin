 ...
 * service repo, evals, and tests set UIZZE_CATALOG_DIR), then the roll
 * API at uizze.style, then a degraded assignment-only seed when both are
 ...
 * never leave the machine. DO_NOT_TRACK or UIZZE_NO_TELEMETRY disables
 * the ping entirely.
 *
 * Env vars:
 *   UIZZE_CONCEPT_SEED — same as --from; for reproducible eval runs.
 *   UIZZE_CATALOG_DIR  — directory holding the four catalog JSON files.
 *   UIZZE_API_URL      — roll API base (default https://uizze.style/api).
 *   UIZZE_NO_TELEMETRY — disables the choice ping (DO_NOT_TRACK also honored).
 */
...
// tests point UIZZE_CATALOG_DIR at one), then the roll API, then a
// degraded assignment-only seed. The full catalog does not ship with the skill.
const CATALOG_DIR = process.env.UIZZE_CATALOG_DIR || here;
const API_BASE = (
  process.env.UIZZE_API_URL || "https://uizze.style/api"
).replace(/\/$/, "");
const API_TIMEOUT_MS = Number(process.env.UIZZE_API_TIMEOUT || 4000);
...
function requireLocalConcepts() {
  ...
  if (!local) {
    throw new Error(
      "concept-seed: no local catalog (set UIZZE_CATALOG_DIR or pass sourceConcepts)",
    );
  }
  return local;
}
...
function telemetryDisabled() {
  return Boolean(process.env.UIZZE_NO_TELEMETRY || process.env.DO_NOT_TRACK);
}
...
const CARD_BASE =
  process.env.UIZZE_CARD_BASE || "https://uizze.style/worlds/cards";
...
export function renderConceptSeed({
  scope = "surface",
  key = process.env.UIZZE_CONCEPT_SEED || crypto.randomBytes(4).toString("hex"),
  reroll = 0,
  register = null,
  mode = null,
  grain = null,
  ...
  for (
    let draw = 0;
    scope === "surface" && dealtIndices.length < Math.min(3, candidateCount);
    draw += 1
  ) {
    const idx =
  ...
  it is not the assigned direction, kicker UIZZE’S PICK, with an honest risk line
  ...
  comp-led default (.uizze/config.json buildPath; the page toggle
...
network contact is one GET to https://uizze.style/api/roll whose query
...
`;
  ...
  // widens it. UIZZE_COMPOSITIONS=1 re-enables rendering for catalog
  // development; the draw machinery, axes, and grain report stay intact.
  const compositionsEnabled = process.env.UIZZE_COMPOSITIONS === "1";
      ...
      process.stdout.write(
        await renderConceptSeed({
          ...
          key:
            fromIdx !== -1
              ? args[fromIdx + 1]
              : process.env.UIZZE_CONCEPT_SEED ||
