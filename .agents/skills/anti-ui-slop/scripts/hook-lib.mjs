/**
 * Shared library for the Uizze design hook.
 *
...
export const ENVELOPE_PREFIX = "[uizze@1]";
...
// behavior with `.uizze/config.json` → `hook: { "perEditRules": "all" }`.
...
// A project opts back in with `.uizze/config.json`:
...
export const HOOK_LOCAL_IGNORE_PATTERNS = Object.freeze([
  ".uizze/hook.cache.json",
  ".uizze/hook.pending.json",
  ".uizze/config.local.json",
]);

const HOOK_IGNORE_MARKER_OPEN = "# uizze-hook-ignore-start";
const HOOK_IGNORE_MARKER_CLOSE = "# uizze-hook-ignore-end";
...
export function getConfigPath(cwd) {
  return path.join(cwd, ".uizze", "config.json");
}

export function getLocalConfigPath(cwd) {
  return path.join(cwd, ".uizze", "config.local.json");
}

export function getCachePath(cwd) {
  return path.join(cwd, ".uizze", "hook.cache.json");
}

export function getPendingPath(cwd) {
  return path.join(cwd, ".uizze", "hook.pending.json");
}
...
function looksLikeProjectRoot(dir) {
  return [".git", "package.json", ".uizze"].some((marker) => {
    try {
...
// Where `.uizze/` (cache + config) lives for this event. Normally the
...
// .uizze), key to the edited file's nearest project root instead, so a
...
export function suppressionNotice(filePath) {
  return `${ENVELOPE_PREFIX} Suppressing further design hints on ${filePath}. More than ${EDIT_COUNT_THRESHOLD} edits in this session reached. Run ${UIZZE_COMMAND} audit to revisit.`;
}
...
export function renderTemplate(findings, filePath, config, opts = {}) {
  ...
  const more =
    remaining > 0
      ? `... and ${remaining} more (see ${UIZZE_COMMAND} audit).`
      : null;
...
function renderGroupedTemplate(groups, config, opts = {}) {
  ...
  for (const group of realGroups) {
    ...
    if (hidden > 0) {
      lines.push(
        `- ... ${hidden} more in ${display} (see ${UIZZE_COMMAND} audit).`,
      );
    }
  }
...
function clampGroupedToBudget(header, lines, footer, maxChars) {
  const assemble = (linesArr, omitted, footerText) =>
    [
      header,
      ...linesArr,
      ...(omitted ? [`... and more (see ${UIZZE_COMMAND} audit).`] : []),
      "",
      footerText,
    ].join("\n");
...
function clampToBudget(header, lines, more, footer, maxChars) {
  ...
  for (const footerText of footerFallbacks(footer)) {
    ...
    while (assembled.length > maxChars && working.length > 1) {
      working.pop();
      moreText = `... and more (see ${UIZZE_COMMAND} audit).`;
...
export function resolveHarness(env = {}, event = null) {
  const explicit = env?.UIZZE_HOOK_HARNESS;
...
// silence-on-clean can set `UIZZE_HOOK_QUIET=1` — runHook checks that
...
const STEER_LINE =
  "That does not mean the design is good: keep following the project design system and the uizze skill guidance.";
...
const DESIGN_STALE_NOTE = `${ENVELOPE_PREFIX} DESIGN.md is newer than .uizze/design.json. Run ${UIZZE_COMMAND} document to refresh the design-system sidecar.`;
...
function directiveFooter(opts = {}) {
  ...
  return [
    ...
    `Self-serve ends at ignore-value: \`ignore-file\` and \`ignore-rule\` need the user's explicit approval, and never add an ignore to push a blocked write through. Full suppression ladder: ${UIZZE_COMMAND} hooks.`,
  ].join("\n");
}
...
export async function runHook({
  ...
  try {
    // Re-entrancy guard.
    if (depthIsSet(env.UIZZE_HOOK_DEPTH) || depthIsSet(env.CLAUDE_HOOK_DEPTH)) {
      return result({ reentrant: true, durationMs: 0 });
    }

    if (truthy(env.UIZZE_HOOK_DISABLED)) {
    ...
    let skippedBytes = 0;
    const quietMode = truthy(env.UIZZE_HOOK_QUIET) || config.quiet === true;
    ...
    if (freshGroups.length > 0) {
      ...
      // `.uizze/`: dedup, suppression, and the notice flags need it.
    ...
    // `.uizze/` dir marks a project that opted in. A non-UI edit, or a
    // clean UI edit in a project with no Uizze footprint, must be a
    // no-op on disk (issues #344, #305).
    if (
      deferredTotal > 0 ||
      (cacheDirty && fs.existsSync(path.join(projectCwd, ".uizze")))
    ) {
      persistCache(projectCwd, cache);
    }
