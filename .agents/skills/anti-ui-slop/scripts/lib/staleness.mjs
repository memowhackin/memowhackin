/**
 * Staleness detection for Uizze's own project artifacts: PRODUCT.md,
 * DESIGN.md and its `.uizze/design.json` sidecar, `.uizze/config.json`,
 ...
 *   2. Schema drift. An artifact was written by an older Uizze: fields it
...
const DIRECTION_WORK_PATHS = Object.freeze([
  path.join(".uizze", "surfaces"),
  path.join(".uizze", "mocks", "decision"),
]);
 ...
 * uizze-paths (which resolves the project root) and context.mjs (which
 * cannot import uizze-paths without a cycle) share one definition of
...
export function designSidecarCandidatesFor(
  ...
  const candidates = [
    path.join(projectRoot, ".uizze", "design.json"),
 ...
 *
 * `sidecarCandidates` comes from uizze-paths' resolver so this module
...
export function checkConfig({ projectRoot, repoRoot }) {
  ...
  for (const root of roots) {
    for (const name of ["config.json", "config.local.json"]) {
      const filePath = path.join(root, ".uizze", name);
  ...
  return [
    finding({
      ...
      fix:
        ...
        'Write the answer to `.uizze/config.json` as `"buildPath": "comp"` or `"buildPath": "code"`, ' +
...
export function checkProjectRoots({
  patterns = [],
  candidates = [],
  configuredIn = ".uizze/config.json",
}) {
