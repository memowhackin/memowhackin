...
function cachePath() {
  return (
    process.env.UIZZE_STALENESS_CACHE ||
    path.join(os.homedir(), ".uizze", "staleness-check.json")
  );
}
 ...
 * the file accumulate one entry per directory Uizze has ever booted in
...
/**
 * Opt out with UIZZE_NO_STALENESS_CHECK=1 or `"stalenessCheck": false` in
 * .uizze/config.json. Local config overrides shared, matching how
 * updateCheck resolves.
 */
export function stalenessCheckDisabled(roots = [process.cwd()]) {
  if (process.env.UIZZE_NO_STALENESS_CHECK) return true;
  let value;
...
export function buildStalenessDirective(findings) {
  ...
  const lines = [
    ...
    "Uizze's own project files have drifted from what this version reads. " +
