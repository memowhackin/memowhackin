...
export function buildCopyEditBatchPrompt(batch, { cwd = process.cwd() } = {}) {
  ...
  const repairLines = compactBatch.repair
    ...
    : [];
  return [
    "You are the Uizze staged copy-edit batch applier.",
    "",
    ...
    "- Never copy browser edit-mode scaffolding into source: no contenteditable, data-uizze-* markers, wrapper variants, generated style/script tags, or runtime-only attributes.",
    ...
    "- If package.json defines scripts.uizze:manual-edit-validate, it must pass after edits.",
    "- Check for leftover uizze-carbonize markers or variant wrapper markers in touched files.",
    "",
...
export async function runCopyEditBatchAgent(batch, opts = {}) {
  ...
  if (provider === "mock") {
    const delayMs = Number(env.UIZZE_LIVE_COPY_AGENT_MOCK_DELAY_MS || 0);
    if (delayMs > 0)
  ...
  const outDir =
    opts.outDir || fs.mkdtempSync(path.join(os.tmpdir(), "uizze-copy-batch-"));
...
export function runCopyEditPostApplyChecks({
  ...
  for (const relativeFile of uniqueFiles) {
    ...
    const markerMatch = findLeftoverUizzeMarker(content);
    if (markerMatch)
      failures.push({
        file: relativeFile,
        reason: "leftover_uizze_marker",
        marker: markerMatch,
      });
...
function findLeftoverUizzeMarker(content) {
  const commentMarker = content.match(
    /^\s*(?:<!--|\{\/\*)\s*uizze-carbonize-(?:start|end)\b|^\s*(?:<!--|\{\/\*)\s*uizze-variants-(?:start|end)\b/m,
  );
  if (commentMarker) return commentMarker[0];

  const attrPattern =
    /\bdata-uizze-(?:variants?|original-text|editable|text-wrap)\s*=/g;
...
function readManualEditValidationScript(cwd) {
  ...
  try {
    ...
    const script = pkg?.scripts?.["uizze:manual-edit-validate"];
...
function stripLiveRuntimeHtml(html) {
  ...
  return html
    .replace(
      /\sdata-uizze-(?:original-text|editable|text-wrap)(?:=(?:"[^"]*"|'[^']*'|[^\s>]+))?/g,
      "",
    )
...
function mockBatchResult(batch, env, cwd = process.cwd()) {
  ...
  const raw = env.UIZZE_LIVE_COPY_AGENT_MOCK_RESULT;
  if (raw) {
    const parsed = parseCopyEditBatchResult(raw);
    if (parsed) return parsed;
    throw new Error("Invalid UIZZE_LIVE_COPY_AGENT_MOCK_RESULT JSON");
  }
  return {
    status: "done",
...
function applyMockWrites(env, cwd) {
  const raw = env.UIZZE_LIVE_COPY_AGENT_MOCK_WRITES;
  if (!raw) return;
  const writes = tryParseJson(raw);
  if (!writes || typeof writes !== "object" || Array.isArray(writes)) {
    throw new Error("Invalid UIZZE_LIVE_COPY_AGENT_MOCK_WRITES JSON");
  }
...
export function chooseCopyEditAgent({
  ...
  const mode = (env.UIZZE_LIVE_COPY_AGENT || "auto").trim().toLowerCase();
...
function runCodex(
  ...
  const args = [
    ...
    resultPath,
    "-c",
    `model_reasoning_effort="${env.UIZZE_LIVE_COPY_AGENT_EFFORT || "low"}"`,
  ];
  if (env.UIZZE_LIVE_COPY_AGENT_MODEL) {
    args.push("--model", env.UIZZE_LIVE_COPY_AGENT_MODEL);
  }
  args.push("-");
...
export function describeNoProviderError({
  ...
  if (chatAvailable()) {
    lines.push(
      "  • Chat: an Uizze live session is polling but selection chose another provider — unexpected; please report.",
    );
  } else {
    lines.push(
      "  • Chat: no Uizze live session is currently polling on this server. Start Uizze live in your chat to route Apply through the chat agent.",
    );
  }
  lines.push(
    "Fix one of the above, or set UIZZE_LIVE_COPY_AGENT=mock for tests.",
  );
  return lines.join("\n");
}
