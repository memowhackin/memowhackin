/**
 * Uizze Live Variant Mode - Browser Script
 *
 * Injected into the user's page via <script src="http://localhost:PORT/live.js">.
 * The server prepends window.__UIZZE_TOKEN__ and window.__UIZZE_PORT__
 * before this code.
 *
...
(function () {
  ...
  if (window.__UIZZE_LIVE_INIT__) return;
  window.__UIZZE_LIVE_INIT__ = true;

  const TOKEN = window.__UIZZE_TOKEN__;
  const PORT = window.__UIZZE_PORT__;
  const APP_ROOT = window.__UIZZE_APP_ROOT__ || null;
  if (!TOKEN || !PORT) {
    window.__UIZZE_LIVE_INIT__ = false; // reset so the real load can init
    return;
  }
  ...
  const PREFIX = "uizze-live";
  const UIZZE_COMMAND = (window.__UIZZE_COMMAND_PREFIX__ || "/") + "uizze";
  const PICK_CURSOR_STYLE_ID = PREFIX + "-pick-cursor-style";
  const MANUAL_APPLY_STATE_TTL_MS = 15 * 60 * 1000;
  const sessionState =
    window.__UIZZE_LIVE_SESSION__?.createLiveBrowserSessionState({
      prefix: PREFIX,
  ...
  if (!sessionState) {
    console.error(
      "[uizze] live-browser-session.js was not loaded. Live mode cannot start safely.",
    );
    window.__UIZZE_LIVE_INIT__ = false;
    return;
  }
  ...
  // window.__UIZZE_VOCAB__ when it serves /live.js (same injection path as
  ...
  const VOCAB = Array.isArray(window.__UIZZE_VOCAB__)
    ? window.__UIZZE_VOCAB__
    : [];
  const ICONS = {};
  ...
  // script, so it cannot import that module; the private uizze-site repo
  ...
  const LIVE_CHROME_MOUNT_CONTRACT = Array.isArray(
    window.__UIZZE_LIVE_MOUNT_CONTRACT__,
  )
    ? window.__UIZZE_LIVE_MOUNT_CONTRACT__
    : ["root", "transport", "state", "actions"];
  const LIVE_UI_SURFACES = Array.isArray(window.__UIZZE_LIVE_UI_SURFACES__)
    ? window.__UIZZE_LIVE_UI_SURFACES__
    : [];
  ...
  const SCROLL_ANCHOR_LOCK_ID = "uizze-scroll-anchor-lock";
  const VARIANT_STATE_STYLE_ID = "uizze-variant-state";
  const DISCARD_STATE_STYLE_ID = "uizze-discard-state";
  ...
  const domHelpers = window.__UIZZE_LIVE_DOM__?.createLiveBrowserDomHelpers({
    prefix: PREFIX,
    skipTags: SKIP_TAGS,
    document,
  });
  if (!domHelpers) {
    console.error(
      "[uizze] live-browser-dom.js was not loaded. Live mode cannot start safely.",
    );
    window.__UIZZE_LIVE_INIT__ = false;
    return;
  }
  ...
  window.__UIZZE_LIVE_CHROME_CORE__ = {
    version: 1,
    adapter: window.__UIZZE_LIVE_ADAPTER__ || "dom",
  ...
  function showHighlight(el) {
    ...
    if (el.hasAttribute?.("data-uizze-insert-placeholder")) return;
  ...
  function onAnnotDown(e) {
    ...
    const resizeEdge = e.target.closest?.("[data-uizze-placeholder-resize]")
      ?.dataset.uizzePlaceholderResize;
  ...
  function stripManualEditRuntimeState(root) {
    ...
    const nodes = [
      ...
      ...root.querySelectorAll(
        "[data-uizze-editable], [data-uizze-original-text], [data-uizze-text-wrap]",
      ),
    ];
    for (const node of nodes) {
      const runtimeEditable =
        node.hasAttribute("data-uizze-editable") ||
        node.hasAttribute("data-uizze-original-text");
      node.removeAttribute("data-uizze-editable");
      node.removeAttribute("data-uizze-original-text");
      node.removeAttribute("data-uizze-text-wrap");
      if (runtimeEditable) {
  ...
  function selectionTagLabel(el) {
    if (!el) return "";
    if (el.hasAttribute?.("data-uizze-insert-placeholder")) return "slot";
  ...
  function applyPlaceholderSizingStyles(placeholder, sizing) {
    placeholder.dataset.uizzePlaceholderWidth = sizing.kind;
  ...
  function materializePlaceholderWidth(placeholder) {
    ...
    const kind = placeholder.dataset.uizzePlaceholderWidth;
    ...
    placeholder.dataset.uizzePlaceholderWidth = "explicit";
  }
  ...
  function setLiveState(next) {
    state = next;
    window.__UIZZE_LIVE_STATE__ = next;
    syncPageInteractionCursor();
  ...
  function resolveBarAnchor() {
    ...
    if (currentSessionId && (state === "GENERATING" || state === "CYCLING")) {
      const wrapper = document.querySelector(
        '[data-uizze-variants="' + currentSessionId + '"]',
      );
      if (wrapper) {
        const variantCount = wrapper.querySelectorAll(
          '[data-uizze-variant]:not([data-uizze-variant="original"])',
        ).length;
  ...
  function isInsertGeneratingSession() {
    ...
    return !!wrapper && wrapper.dataset.uizzeMode === "insert";
  }
  ...
  function buildPlaceholderResizeHandles() {
    ...
    for (const spec of specs) {
      ...
      handle.dataset.uizzePlaceholderResize = spec.edge;
  ...
  function createInsertPlaceholder(anchor, position, layoutAxis) {
    ...
    placeholder.setAttribute("data-uizze-insert-placeholder", "true");
  ...
  function ensureConfigureInputStyle() {
    ...
    s.textContent =
      "@keyframes uizze-configure-voice-pulse { 0%, 100% { opacity: 0.55; } 50% { opacity: 1; } }" +
      "#" +
      PREFIX +
      "-input, #" +
      PREFIX +
      ...
      PREFIX +
      '-insert-voice[data-listening="true"] svg { animation: uizze-configure-voice-pulse 1.1s ease-in-out infinite; }' +
    ...
    if (hasParams || paramsPending) {
      ...
      if (paramsPending) {
        const spinner = el("span", {
          ...
          animation: "uizze-spin 0.6s linear infinite",
          boxSizing: "border-box",
          flexShrink: "0",
        });
  ...
  function recoverEmptyCycling(reason) {
    ...
    try {
      console.warn(
        "[uizze] Refusing to render empty variant cycling state:",
        reason,
      );
  ...
  // variant wrapper:
  //
  //   <div data-uizze-variant="1"
  //        data-uizze-params='[{"id":"density","kind":"steps",...}]'>
  //
  ...
  function getVisibleVariantEl() {
    ...
    return wrapper.querySelector(
      '[data-uizze-variant="' + visibleVariant + '"]',
    );
  }

  function parseVariantParams(variantEl) {
    // Svelte component variants can't carry a `data-uizze-params` attribute:
    ...
    if (!variantEl) return [];
    const raw = variantEl.getAttribute("data-uizze-params");
    if (!raw) return [];
    try {
      const parsed = JSON.parse(raw);
      return Array.isArray(parsed) ? parsed : [];
    } catch (err) {
      console.warn("[uizze] Invalid data-uizze-params JSON:", err.message);
      return [];
    }
  }
  ...
  function applyParamValue(variantEl, param, value) {
    ...
    // [data-uizze-component-mount] with no [data-uizze-variant="N"]
  ...
  function wrapMixedContentTextNodes(rootEl) {
    ...
    if (hasText && hasElement) {
      for (const node of children) {
        if (node.nodeType === 3 && /\S/.test(node.nodeValue || "")) {
          ...
          wrap.dataset.uizzeTextWrap = "true";
    ...
    for (const child of Array.from(rootEl.children)) {
      if (!child.dataset || !child.dataset.uizzeTextWrap) {
  ...
  function unwrapMixedContentTextNodes(rootEl) {
    if (!rootEl || rootEl.nodeType !== 1) return;
    const wraps = rootEl.querySelectorAll('[data-uizze-text-wrap="true"]');
  ...
  function enableInlineEdit(targetEl) {
    ...
    for (const row of rows) {
      ...
      row.el.dataset.uizzeEditable = "true";
      row.el.dataset.uizzeOriginalText = row.text;
  ...
  function disableInlineEdit(opts = {}) {
    for (const row of inlineEditRows) {
      ...
      delete row.el.dataset.uizzeEditable;
      delete row.el.dataset.uizzeOriginalText;
  ...
  function restoreInlineEditDrafts() {
    for (const row of inlineEditRows) {
      if (inlineEditDrafts.has(row.el)) {
        row.el.textContent = row.el.dataset.uizzeOriginalText;
      }
    }
  }
  ...
  function documentRefClassSuffix(el) {
    ...
    for (const cls of el.classList) {
      if (!cls || cls.indexOf("uizze-") === 0) continue;
  ...
  function copyEditLeafContext(el, originalText, newText) {
    ...
    return {
      ...
      id: el.id || null,
      classes: el.classList
        ? [...el.classList].filter((cls) => cls.indexOf("uizze-") !== 0)
        : [],
      originalText,
      newText,
      textContent: (el.textContent || "").slice(0, 500),
    ...
    for (const row of rows || []) {
      ...
      out.push({
        ...
        classes: row.el?.classList
          ? [...row.el.classList].filter((cls) => cls.indexOf("uizze-") !== 0)
          : [],
        text,
      });
  ...
  async function applyEditing() {
    ...
    try {
      ...
      console.error("[uizze] manual edit stash failed:", err);
  ...
  function ensureSpinKeyframes() {
    ...
    style.textContent =
      "@keyframes uizze-spin { to { transform: rotate(360deg); } }";
    uiAppendStyle(style);
  }
  ...
  async function fetchPendingCount() {
    try {
      ...
      console.warn("[uizze] failed to fetch pending count:", err);
    }
  }
  ...
  async function onPendingPillClick() {
    ...
    try {
      ...
      if (result.failed && result.failed.length > 0) {
        console.warn("[uizze] some copy edits failed:", result.failed);
        showToast(
          "Applied " +
        ...
        if (n > 0) {
          ...
          console.warn("[uizze] apply returned no verified edits:", result);
          showToast("No edits applied - see console", 4000);
        }
      }
    } catch (err) {
      console.error("[uizze] commit failed:", err);
  ...
  async function onPendingTrashClick() {
    ...
    try {
      ...
      console.error("[uizze] discard failed:", err);
  ...
  async function onPendingKeepFixingClick() {
    ...
    try {
      ...
      console.error("[uizze] repair retry failed:", err);
  ...
  async function onPendingRollbackClick() {
    ...
    try {
      ...
      console.error("[uizze] manual Apply rollback failed:", err);
  ...
  function restoreDiscardedManualEdits(entries) {
    ...
    if (failures > 0) {
      console.warn(
        "[uizze] skipped unsafe copy edit DOM restore for",
        failures,
  ...
  function directMixedTextRestoreNodes(parent) {
    return Array.from(parent?.childNodes || []).filter((node) => {
      ...
      return (
        node.nodeType === 1 &&
        node.dataset &&
        node.dataset.uizzeTextWrap === "true" &&
  ...
  function syncEditBadgeHitProxies() {
    ...
    for (const target of targets) {
      ...
      if (!proxy) {
        ...
        proxy.type = "button";
        proxy.tabIndex = -1;
        proxy.dataset.uizzeEditBadgeProxy = "true";
  ...
  function initEditBadge() {
    ...
    if (!uiGetById(PREFIX + "-edit-badge-focus-style")) {
      ...
      s.textContent =
        ...
        '[data-uizze-editable="true"] { outline: none !important; box-shadow: none !important; }' +
        '[data-uizze-editable="true"]:focus { outline: none !important; box-shadow: none !important; }' +
        '[data-uizze-editable="true"]:focus-visible { outline: none !important; box-shadow: none !important; }';
      uiAppendStyle(s);
    }
  }
  ...
  function parseOriginalMarkupElement(originalMarkup) {
    ...
    const doc = parser.parseFromString(
      '<div id="uizze-anchor">' + originalMarkup + "</div>",
      "text/html",
    );
    return doc.getElementById("uizze-anchor")?.firstElementChild || null;
  }
  ...
  function isUsableInjectionAnchor(el) {
    return (
      ...
      !own(el) &&
      !el.closest?.("[data-uizze-variants]")
    );
  }
  ...
  function waitForVariantAnchorAndRetry({
    ...
    const origContent = srcWrapper?.querySelector(
      '[data-uizze-variant="original"] > :first-child',
    );
  ...
  async function probePreviewTree(manifest) {
    ...
    try {
      ...
      return { ok: false, tried: err.uizzeTriedUrls || candidates };
    }
  }

  function loadSvelteRuntime(runtimeModule, manifest) {
    const modulePath = runtimeModule || "/src/lib/uizze/__runtime.js";
  ...
  // componentDir (keyed by variant number), because a `data-uizze-params`
      ...
      if (entry.kind === "collection" && entry.item && entry.item.rootTag) {
        ...
        values[entry.prop] = matches.map((itemEl, index) => {
          ...
          if (entry.item.keyField && item[entry.item.keyField] == null) {
            item[entry.item.keyField] = "uizze-live-" + index;
          }
          return item;
        });
  ...
  function cloneWithoutElements(rootEl, excludedEls) {
    ...
    const MARK = "data-uizze-hydration-excluded";
  ...
  async function mountSvelteComponentVariant(variantNum) {
    ...
    try {
      ...
      console.error(
        "[uizze] Failed to mount component variant " +
          variantNum +
          " for " +
          sessionId +
          ":",
  ...
  async function injectSvelteComponentsFromManifest(manifestPath, sessionId) {
    ...
    try {
      ...
      if (!liveEl?.parentElement) {
        console.warn("[uizze] Could not find original element in live DOM.");
        arrivedVariants = availableVariants;
        expectedVariants =
          Number(manifest.count) || expectedVariants || arrivedVariants;
      ...
      wrapper.dataset.uizzeVariants = sessionId;
      wrapper.dataset.uizzeVariantCount = String(
        manifest.count || expectedVariants || 1,
      );
      wrapper.dataset.uizzePreview = componentPreviewMode;
      wrapper.style.display = "contents";

      const mountTarget = document.createElement("div");
      mountTarget.dataset.uizzeComponentMount = sessionId;
      ...
      console.error("[uizze] Failed to mount component-preview variants:", err);
  ...
  function renderMountErrorCard() {
    ...
    retry.dataset.uizzeMountRetry = "true";
  ...
  function abortSvelteComponentInjection(sessionId, details) {
    try {
      ...
      console.warn("[uizze] Svelte component abort cleanup failed:", err);
    }
    hideShaderOverlay();
    if (pendingSvelteComponentRetryObserver) {
      pendingSvelteComponentRetryObserver.disconnect();
  ...
  function resetSvelteComponentSession(sessionId, message) {
    try {
      ...
      console.warn("[uizze] Svelte component reset cleanup failed:", err);
    }
    hideShaderOverlay();
    if (variantObserver) {
      variantObserver.disconnect();
  ...
  function discardOrphanedSession(reason) {
    ...
    if (!sessionId) return;
    console.warn(
      "[uizze] Discarding orphaned session " + sessionId + ": " + reason,
    );
  ...
  function injectVariantsFromSource(filePath, sessionId, opts = {}) {
    ...
    fetch(url)
      ...
      .then((html) => {
        ...
        const startMark = "<!-- uizze-variants-start " + sessionId + " -->";
        const endMark = "<!-- uizze-variants-end " + sessionId + " -->";
        ...
        if (!srcWrapper) {
          console.warn("[uizze] Variant wrapper not found in source file.");
        ...
        if (existingWrapper) {
          ...
          if (!liveEl) {
            console.warn(
              "[uizze] Could not find original element in live DOM.",
            );
            enterRecoveryWaitingForAnchor({
              filePath,
              sessionId,
        ...
        expectedVariants = parseInt(
          wrapper.dataset.uizzeVariantCount || arrivedVariants,
        );
        if (arrivedVariants <= 0) {
          if (state === "GENERATING") {
            ...
            if (!opts.generationCompleted) {
              console.log(
                "[uizze] Source has scaffold but no variants yet; still generating.",
              );
              return;
            }
            ...
            if (attempt < COMPLETED_SOURCE_FALLBACK_RETRIES) {
              console.log(
                "[uizze] Generation is done but source shows no variants yet; retrying read (" +
                  (attempt + 1) +
                  "/" +
        ...
        if (parameterGenerationState === "loading")
          completeParameterPublication();
        console.log(
          "[uizze] Injected " + arrivedVariants + " variants from source file.",
        );
      })
      .catch((err) => {
        console.error("[uizze] Failed to fetch source:", err);
  ...
  function updateSelectedElement() {
    ...
    if (svelteComponentSession?.sessionId === currentSessionId) {
      const anchor = resolveSvelteComponentAnchor();
      if (anchor && !anchor.__uizzeFrozenAnchor) selectedElement = anchor;
      return;
    }
    const wrapper = document.querySelector(
      '[data-uizze-variants="' + currentSessionId + '"]',
  ...
  function readVisibleVariantFromDOM(sessionId) {
    ...
    for (const variant of variants) {
      ...
      const idx = parseInt(variant.dataset.uizzeVariant || "0", 10);
      if (idx > 0) return idx;
    }
    return 0;
  }
  ...
  // single variant (`… > [data-uizze-variant="N"]`); without it, targets
  // every variant via the bare `[data-uizze-variant]` attribute.
  function variantStateSelector(sessionId, num) {
    const wrapper = '[data-uizze-variants="' + sessionId + '"]';
    const variant =
      num == null
        ? "[data-uizze-variant]"
        : '[data-uizze-variant="' + num + '"]';
  ...
  function showOriginalDuringDiscard(sessionId) {
    ...
    styleEl.textContent =
      wrapper +
      ' > [data-uizze-variant]:not([data-uizze-variant="original"]) { display:none !important; }\n' +
      wrapper +
      ' > [data-uizze-variant="original"] { display:block !important; }';
  }
  ...
  function startScrollLock(sessionId, initialTargetY, initialAnchorTop) {
    ...
    scrollLockObserver = new MutationObserver((mutations) => {
      for (const m of mutations) {
        if (m.target?.closest?.('[data-uizze-variants="' + sessionId + '"]')) {
        ...
        for (const n of m.addedNodes) {
          if (
            n.nodeType === 1 &&
            (n.matches?.('[data-uizze-variants="' + sessionId + '"]') ||
              n.querySelector?.('[data-uizze-variants="' + sessionId + '"]'))
          ) {
  ...
  function startVariantObserver(sessionId) {
    ...
    const obs = new MutationObserver((mutations) => {
      ...
      // Only react to mutations that add nodes with data-uizze-variant,
      // or mutations inside the variant wrapper. Ignore our own bar/UI changes.
      let dominated = false;
      for (const m of mutations) {
        if (m.target.closest?.("[data-uizze-variants]")) {
          dominated = true;
          break;
        }
        for (const n of m.addedNodes) {
          ...
          if (n.dataset?.uizzeVariants || n.dataset?.uizzeVariant) {
            dominated = true;
            break;
          }
          ...
          if (n.querySelector?.("[data-uizze-variants],[data-uizze-variant]")) {
            dominated = true;
            break;
          }
        }
      ...
      if (selectedElement && !document.body.contains(selectedElement)) {
        const isInsert = wrapper.dataset.uizzeMode === "insert";
        if (isInsert) {
          const visEl =
      ...
      const expected = parseInt(wrapper.dataset.uizzeVariantCount || "0");
      ...
      if (arrivedVariants > 0) {
        ...
        recoveryWaitingForAnchor = false;
        hideShaderOverlay();
        if (wrapper.dataset.uizzeMode === "insert") finalizeInsertSession();
        updateSelectedElement();
        showOrUpdateCyclingBar();
        disableInlineEdit();
        if (arrivedVariants >= expectedVariants && expectedVariants > 0)
  ...
  function startScrollTracking() {
    function tick() {
      if (
        ...
        if (
          hiTarget &&
          !hiTarget.hasAttribute?.("data-uizze-insert-placeholder")
        ) {
  ...
  function connectSSE() {
      ...
      switch (msg.type) {
        case "connected":
          ...
          if (!hasProjectContext)
            showToast(
              `No PRODUCT.md found. Variants will be brand-agnostic. Run ${UIZZE_COMMAND} init to generate one.`,
              7000,
            );
          console.log("[uizze] Live mode connected.");
        ...
        case "error":
          ...
          if (awaitingAcceptResult?.id && msg.id === awaitingAcceptResult.id) {
            awaitingAcceptResult = null;
            console.error("[uizze] Accept failed after teardown:", msg.message);
          ...
          if (maybeCompleteSteer(msg)) break;
          console.error("[uizze] Error:", msg.message);
    ...
    evtSource.onerror = () => {
      ...
      if (sseRetries <= SSE_MAX_RETRIES) {
        console.log(
          "[uizze] SSE connection lost. Retry " +
            sseRetries +
            "/" +
            SSE_MAX_RETRIES +
            "...",
      ...
      console.log("[uizze] Live server unreachable. Cleaning up UI.");
      evtSource.close();
      evtSource = null;
      handleServerLost();
    };
  ...
  function sendEvent(msg, opts) {
    ...
    function handleFailure(err) {
      if (opts && opts.throwOnError) {
        console.error("[uizze] Failed to send event:", err);
        throw err;
      }
      console.debug("[uizze] Dropped optional live event:", err);
      return null;
    }
    // Token in the query string as well as the body: the URL token is what
    // authorizes the CORS preflight when the page runs on a non-loopback
  ...
  function abandonForeignSession(sessionId) {
    ...
    abandonedForeignSessionId = sessionId;
    console.warn(
      "[uizze] The live server has no record of session " +
        sessionId +
  ...
  function handleKeyDown(e) {
    ...
    if (e.target.isContentEditable && isInlineEditActive(e.target)) {
      ...
      e.preventDefault();
      e.stopPropagation();
      const original = e.target.dataset.uizzeOriginalText;
  ...
  function shouldUseAncestorCropShaderProxy(el) {
    ...
    const adapter = String(window.__UIZZE_LIVE_ADAPTER__ || "").toLowerCase();
    ...
    if (
      ...
      return true;
    const wrapper = el?.closest?.("[data-uizze-variants]");
    return isFrameworkComponentPreviewMode(wrapper?.dataset?.uizzePreview);
  }
    ...
    try {
      ...
      if (shouldUseAncestorCropShaderProxy(el)) {
        try {
          ...
          console.warn(
            "[uizze] Svelte ancestor crop capture failed, falling back to element capture:",
            err,
          );
        }
      }
  ...
  async function captureAndEmit(el, basePayload, snapshot, rect) {
    ...
    try {
      ...
      console.warn(
        "[uizze] capture failed, proceeding without screenshot:",
        err,
      );
    }
    ...
    if (blob && hasAnnotations) {
      try {
        ...
        if (uploadRes.ok) {
          ...
          console.warn("[uizze] annotation upload failed:", uploadRes.status);
        }
      } catch (err) {
        console.warn("[uizze] annotation upload failed:", err);
      }
    }
...
void main() {
  ...
  async function showShaderOverlay(el, blob, rect, paper) {
    ...
    try {
      ...
      console.warn("[uizze] shader setup failed:", err);
      canvas.remove();
      return;
    }
    ...
    try {
      ...
      console.warn("[uizze] shader bitmap decode failed:", err);
  ...
  async function handleAccept() {
    ...
    const acceptedIsSvelteComponent =
      ...
      isFrameworkComponentPreviewMode(acceptWrapper?.dataset?.uizzePreview);
    ...
    sendEvent(acceptPayload, { throwOnError: true })
      .then(() => {
        ...
        document.documentElement.dataset.uizzeAcceptToPickingMs = String(
  ...
  function acceptedDomAlreadyClean(pending) {
    ...
    return (
      ...
      matches.every(
        (el) =>
          !el.closest(
            "[data-uizze-variants],[data-uizze-variant],[data-uizze-carbonize]",
          ),
      )
    );
  }
  ...
  function findAcceptedRuntimeWrappers(sessionId) {
    ...
    return [
      ...new Set([
        ...
        ...document.querySelectorAll(
          '[data-uizze-carbonize="' + sessionId + '"]',
        ),
      ]),
    ];
  }
  ...
  function reloadAfterMissingAcceptedDom(pending) {
    ...
    if (
      pending?.id &&
      document.querySelector('[data-uizze-variants="' + pending.id + '"]')
    )
      return;
    location.reload();
  }
  ...
  function commitAcceptedVariantToDom(sessionId, variantId) {
    ...
    const style = wrapper.querySelector("style[data-uizze-css]");
    if (
      style &&
      !document.querySelector(
        'style[data-uizze-accepted-css="' + sessionId + '"]',
      )
    ) {
      const promotedStyle = style.cloneNode(true);
      promotedStyle.setAttribute("data-uizze-accepted-css", sessionId);
  ...
  function restoreFromActiveSessions(activeSessions, reason) {
    const wrapper = document.querySelector("[data-uizze-variants]");
    if (
      wrapper &&
      !isFrameworkComponentPreviewMode(wrapper.dataset.uizzePreview)
    )
      return false;
  ...
  function recoverMissedGenerationCompletion(activeSessions) {
    ...
    if (!file) return;
    console.log(
      "[uizze] Reconnected after generation completed; recovering variants from source.",
    );
    setTimeout(() => {
  ...
  function loadSession() {
    ...
    if (saved?.appRoot && APP_ROOT && saved.appRoot !== APP_ROOT) {
      console.warn(
        "[uizze] Ignoring saved live session from another project (" +
          saved.appRoot +
          ").",
      );
  ...
  function cleanup(options) {
    ...
    if (svelteComponentSession?.sessionId === cleanupSessionId) {
      ...
      const wrapper = document.querySelector(
        '[data-uizze-variants="' + cleanupSessionId + '"]',
      );
      if (wrapper) {
  ...
  // If a [data-uizze-variants] wrapper exists in the DOM, the agent wrote
  ...
  function resumeSession() {
    ...
    const sessionId = wrapper.dataset.uizzeVariants;
    ...
    // wrapper holds a single mount target, not [data-uizze-variant] nodes,
    ...
    if (isFrameworkComponentPreviewMode(wrapper.dataset.uizzePreview)) {
      if (!svelteComponentSession?.mountedVariant) {
        return true;
      }
      currentSessionId = sessionId;
      expectedVariants =
        Number(wrapper.dataset.uizzeVariantCount) ||
    ...
    currentSessionId = sessionId;
    expectedVariants = parseInt(wrapper.dataset.uizzeVariantCount || "0");
    const variants = wrapper.querySelectorAll(
      '[data-uizze-variant]:not([data-uizze-variant="original"])',
    );
    arrivedVariants = variants.length;
      ...
      if (shaderTarget) {
        (async () => {
          try {
            ...
            console.warn("[uizze] shader resume failed:", err);
          }
        })();
      }
    }
  ...
  const PICK_PREFS_KEY = "uizze-live-pick";
  const INTERACTION_PREFS_KEY = "uizze-live-interaction";
  ...
  function detectPageTheme() {
    try {
      // Dev override: set localStorage 'uizze-dev-theme' to 'light' or
      // 'dark' to preview the opposite palette without actually changing the
      // page bg. Used for screenshots and theme QA.
      const override = localStorage.getItem("uizze-dev-theme");
  ...
  function attachSteerFocusGuard() {
    if (window.__UIZZE_STEER_FOCUS_GUARD__) return;
    window.__UIZZE_STEER_FOCUS_GUARD__ = true;
  ...
  function steerFocusDebugEnabled() {
    try {
      return localStorage.getItem("uizze-steer-debug") === "1";
    } catch {
      return false;
    }
  }
  ...
  function steerFocusLog(reason, extra) {
    ...
    console.log("[uizze.steer]", reason, {
      state,
      pickActive,
  ...
  function attachSteerFocusDebug() {
    if (!steerFocusDebugEnabled()) return;
    if (window.__UIZZE_STEER_FOCUS_DEBUG__) return;
    window.__UIZZE_STEER_FOCUS_DEBUG__ = true;
    document.addEventListener(
      "focusin",
      (e) => {
    ...
    for (let i = 0; i < 3; i++) {
      wrap.appendChild(
        el("span", {
          ...
          animation:
            "uizze-steer-dot 1.05s ease-in-out " + i * 0.14 + "s infinite",
        }),
      );
    }
    return wrap;
  ...
  function startVoice(ctx) {
    ...
    rec.onerror = (event) => {
      ...
      console.warn("[uizze.voice] recognition error:", code);
    ...
    try {
      ...
      console.warn("[uizze.voice] start failed:", err);
      stopVoice({
        suppressSubmit: true,
  ...
  function initPageChat(parent, P) {
    ...
    if (!uiGetById(PREFIX + "-page-chat-style")) {
      ...
      s.textContent =
        "@keyframes uizze-steer-dot { 0%, 70%, 100% { opacity: 0.28; transform: scale(0.82); } 35% { opacity: 1; transform: scale(1); } }" +
        "@keyframes uizze-steer-processing { 0%, 100% { border-color: oklch(70% 0.12 188 / 0.28); box-shadow: 0 0 0 0 oklch(70% 0.12 188 / 0); } 50% { border-color: oklch(82% 0.07 188 / 0.55); box-shadow: 0 0 14px oklch(70% 0.12 188 / 0.18); } }" +
        "@keyframes uizze-voice-pulse { 0%, 100% { opacity: 0.55; } 50% { opacity: 1; } }" +
        "#" +
        PREFIX +
        '-page-chat[data-processing="true"] { animation: uizze-steer-processing 1.6s ease-in-out infinite; }' +
        "@media (prefers-reduced-motion: reduce) { #" +
        PREFIX +
        ...
        "#" +
        PREFIX +
        '-page-chat-voice[data-listening="true"] svg { animation: uizze-voice-pulse 1.1s ease-in-out infinite; }' +
        "@media (prefers-reduced-motion: reduce) { #" +
        PREFIX +
  ...
  // Uizze mark - same paths as site/components/Header.astro + favicon.svg.
  ...
  function initGlobalBar() {
    ...
    if (!uiGetById(PREFIX + "-bar-focus-style")) {
      ...
      s.textContent =
        ...
        ";" +
        "}" +
        "@keyframes uizze-agent-dot { 0%, 100% { opacity: 0.45; transform: scale(0.9); } 50% { opacity: 1; transform: scale(1); } }" +
        "#" +
        PREFIX +
        '-global-bar-brand[data-agent-connected="false"] [data-agent-dot] { animation: uizze-agent-dot 1.4s ease-in-out infinite; }' +
        "@media (prefers-reduced-motion: reduce) { #" +
        PREFIX +
    ...
    // Brand mark - kinpaku Uizze icon (site header / favicon paths).
    const brand = el("span", {
      display: "inline-flex",
      alignItems: "center",
      justifyContent: "center",
    ...
    brand.setAttribute("aria-label", "Uizze live mode - agent not polling");
  ...
  function updateGlobalBarState() {
    ...
    document.querySelectorAll(".uizze-overlay").forEach((o) => {
      o.style.pointerEvents = pickActive || insertActive ? "none" : "";
    });
    syncPageInteractionCursor();
  }

  let detectReady = false; // true once detect script posts 'uizze-ready'
  ...
  function toggleDetect() {
    ...
    if (detectActive) {
      ...
      window.postMessage({ source: "uizze-command", action: "remove" }, "*");
  ...
  function onDetectMessage(e) {
    ...
    if (e.data.source === "uizze-results") {
  ...
  function teardown() {
    ...
    removeVariantStateStylesheet();
    window.__UIZZE_LIVE_INIT__ = false;
    console.log("[uizze] Live mode exited.");
  }

  //
  // Design System Panel - visualizes the project's .uizze/design.json sidecar
  //

  const DESIGN_PREFS_KEY = "uizze-live-design-panel";
  ...
  let designState = {
    ...
    sidecar: null, // .uizze/design.json v2 payload (extensions + components + narrative)
    hasMd: false,
    hasSidecar: false,
  ...
  // Neutral panel palette - deliberately NOT Uizze-branded. The panel is
  // a viewer of the project's design system, not an Uizze surface.
  const DP = {
  ...
  function renderDesignBody() {
    ...
    if (designState.present === false) {
      ...
      empty.innerHTML = `<strong>No DESIGN.md yet</strong>Create one by running <code>${UIZZE_COMMAND} document</code> in your terminal, then re-open this panel.`;
  ...
  function renderStaleHint() {
    ...
    box.innerHTML = `
      ...
      <span class="stale-text"><strong>DESIGN.md is newer than .uizze/design.json.</strong> Run <code>${UIZZE_COMMAND} document</code> to refresh the sidecar.</span>
    `;
    return box;
  }
  ...
  function renderParsedMdCta() {
    ...
    box.innerHTML = `<strong>Basic view</strong>This panel reads the tokens in your <code>DESIGN.md</code> frontmatter. Running <code>${UIZZE_COMMAND} document</code> also generates a <code>.uizze/design.json</code> sidecar with your project's actual component snippets (button, input, nav) and tonal ramps, rendered live below the tokens.`;
    return box;
  }
  ...
  function designEmptyMessage() {
    if (designState.hasMd && !designState.hasSidecar) {
      return (
        ...
        UIZZE_COMMAND +
        " document to generate the .uizze/design.json sidecar."
      );
    }
  ...
  function cssSafe(v) {
    ...
    // .uizze/design.json values rendered into inline style strings.
  ...
  function init() {
    ...
    if (!resumeSession()) {
      console.log(
        "[uizze] Live variant mode ready. Hover over elements to pick one.",
      );
      ...
      const scout = new MutationObserver(() => {
        ...
        if (resumeSession()) {
          console.log(
            "[uizze] Resumed deferred session " +
              currentSessionId +
              " (post-hydration).",
          );
        }
      ...
      console.log(
        "[uizze] Resumed active variant session " +
          currentSessionId +
          " (" +
          arrivedVariants +
          "/" +
