...
import { getLiveDir, safeSessionId } from "./lib/uizze-paths.mjs";
 ...
 * <div data-uizze-carbonize> so the slot keeps a single root node.
 */
function buildCarbonizeReplacement({
  ...
  const pushCarbonizeBody = (bodyIndent) => {
    ...
    lines.push(
      bodyIndent +
        commentSyntax.open +
        " uizze-carbonize-start " +
        id +
        " " +
        commentSyntax.close,
    );
    lines.push(
      bodyIndent + '<style data-uizze-css="' + id + '">' + (isJsx ? "{`" : ""),
    );
  ...
  if (isJsx) {
    ...
    lines.push(
      indent + '<div data-uizze-carbonize="' + id + '" ' + wrapperStyle + ">",
    );
...
function buildAcceptedWrappedSource(
  ...
  const hasHelperAttrs = variantText.includes("data-uizze-variant");
...
function readSourceShadowPreviewMeta(content, id) {
  ...
  const wrapperRe = new RegExp(
    "<[^>]+data-uizze-variants=([\"'])" + escaped + "\\1[^>]*>",
  );
  const match = String(content || "").match(wrapperRe);
  if (!match) return null;
  const tag = match[0];
  if (readHtmlAttr(tag, "data-uizze-preview") !== "source-shadow") return null;
  const sourceFile = readHtmlAttr(tag, "data-uizze-source-file");
  const sourceStartLine = Number(readHtmlAttr(tag, "data-uizze-source-start"));
  const sourceEndLine = Number(readHtmlAttr(tag, "data-uizze-source-end"));
  if (
    !sourceFile ||
...
function findMarkerBlock(id, lines) {
  let start = -1;
  let end = -1;
  const startPattern = "uizze-variants-start " + id;
  const endPattern = "uizze-variants-end " + id;
 ...
 * the `<div data-uizze-variants="ID">` outer wrapper so the picked
 ...
 * (with `data-uizze-variants="ID"`) and its matching `</div>`. We
...
function expandReplaceRange(block, lines, isJsx) {
  ...
  // Walk back for the wrapper `<div data-uizze-variants="..."` opener.
...
function isVariantEndMarkerLine(line, id) {
  return new RegExp(
    "uizze-variants-end\\s+" + escapeRegExp(id) + "(?:\\s|--|\\*/|$)",
  ).test(line);
}
...
function hasVariantWrapperAttr(line, id) {
  const escaped = escapeRegExp(id);
  return new RegExp(
    `data-uizze-variants\\s*=\\s*(?:"${escaped}"|'${escaped}'|\\{["']${escaped}["']\\})`,
  ).test(line);
}
 ...
 *   - CSS `@scope ([data-uizze-variant="N"])` strings that look like the
...
function extractOriginal(lines, block) {
  ...
  const inner = extractInnerByAttr(text, 'data-uizze-variant="original"');
 ...
 *
 * Handles three shapes of `<style data-uizze-css="ID" ...>`:
...
function extractCss(lines, block, id) {
  const styleAttr = 'data-uizze-css="' + id + '"';
  let inStyle = false;
  const content = [];
