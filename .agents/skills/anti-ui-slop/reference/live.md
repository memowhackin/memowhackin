...
1. `live.mjs`: boot. If the request names or implies a file, route, or app inside a monorepo, infer the concrete path and run `node scripts/live.mjs --target <path>` instead; then run the rest of this live session from the returned `projectRoot`. The boot resolves the app root from dev-server config files and persists it in `.uizze/live/roots.json`; every helper re-anchors to that manifest at startup (a wrong cwd cannot fork session state), PRODUCT.md / DESIGN.md are discovered upward to the git root, and relative helper args like `--file` resolve against the app root.
...
3. Poll loop with the default long timeout (600000 ms). Run `live-poll.mjs` again immediately after every event or `--reply`; Codex runs this one-shot poll in the foreground. Never pass a short `--timeout=`. The global bar's **Uizze mark** dims with a pulsing amber dot when nothing is polling `/poll`; restart `live-poll.mjs` to reconnect.
...
7. If interrupted, run `live-status.mjs` or `live-resume.mjs` before guessing. The journal under `.uizze/live/sessions/` is canonical and replays unacknowledged work after a helper restart; the injected `live.js` re-attaches when the page reopens. Fall back to the direct-edit loop only when `live-resume.mjs` reports no active session, never because disconnects felt frequent.
...
The append-only journal under `.uizze/live/sessions/` is canonical durable state (not project source). When the chat was interrupted, polling was missed, the helper restarted, or the browser reloaded:

```bash
node scripts/live-status.mjs      # helper state, active sessions, queued events; works with the helper down
node scripts/live-resume.mjs --id SESSION_ID   # active snapshot, pending event, next safe action
node scripts/live-complete.mjs --id SESSION_ID # canonical manual final acknowledgement after verified cleanup
```
...
<style data-uizze-css="SESSION_ID">
...
<div data-uizze-variant="2" style="display: none">
  <!-- variant 2 -->
</div>
<div data-uizze-variant="3" style="display: none">
  <!-- variant 3 -->
</div>
```
...
**JSX / TSX targets:** wrap `<style>` content in a template literal (CSS braces would parse as JSX), use `className=` / `style={{…}}`, keep `data-uizze-*` attributes as plain strings:

```tsx
<style data-uizze-css="SESSION_ID">{`
  @scope ([data-uizze-variant="1"]) { ... }
`}</style>
<div data-uizze-variant="2" style={{ display: 'none' }}>
  {/* variant 2 */}
</div>
...
2. **Preview in the served file**: manually write the same wrapper scaffold `live-wrap.mjs` produces (`<!-- uizze-variants-start ID --><div data-uizze-variants="ID" data-uizze-variant-count="3" style="display: contents">…</div><!-- end -->`) into the file the browser actually loaded, insert your variant divs, `--reply EVENT_ID done --file <served file>`. This edit is temporary; a regen wiping it is fine.
...
1. **Locate the carbonize block** in `_acceptResult.file`: bracketed by `<!-- uizze-carbonize-start/end SESSION_ID -->` with a `<style data-uizze-css>` element; read the `<!-- uizze-param-values -->` comment first when present, it drives steps 3 and 4.
2. **Move the CSS rules** into the project's real stylesheet (whichever already owns styling for the surrounding element).
3. **Bake param values while rewriting selectors**: retarget `@scope ([data-uizze-variant="N"])` to real semantic classes; keep only the `:scope[data-p-<id>="VALUE"]` branch matching the chosen value; substitute `var(--p-<id>)` literals or update the var's default.
4. **Unwrap the accepted content**: delete the inner variant div (and on JSX the outer `data-uizze-carbonize` div); drop `data-uizze-params` and all `data-p-*` attributes.
...
When native subagents are available, delegate source edits to `uizze_manual_edit_applier` / `uizze-manual-edit-applier`. Pass cwd, scripts path, event id, page URL, chunk/deadline, `batch`, `evidencePath`, and the canonical JSON result schema. The subagent must not poll or reply. If unavailable, apply inline with the same contract.
...
Stops the helper and runs `live-inject.mjs --remove` to strip the injected script (use `stop --keep-inject` to keep it for a quick restart; `.uizze/live/config.json` persists as project config). Then search for and remove any leftover `uizze-variants-start` wrappers and `uizze-carbonize-start` blocks.
